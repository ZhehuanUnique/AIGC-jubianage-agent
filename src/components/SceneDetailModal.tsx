...
function SceneDetailModal({ scene, onClose, onImageUpload, onDelete }: SceneDetailModalProps) {
  const navigate = useNavigate()
  const { projectId } = useParams()
  
  // 检查是否是有效的图片URL（排除默认占位图）
  const hasValidImage = scene.image && 
    !scene.image.startsWith('/scene') && 
    scene.image !== '/scene1.jpg'
  const [imageUrl, setImageUrl] = useState<string | null>(hasValidImage ? scene.image : null)
  ...
  const handleConfirm = () => {
    ...
    // 通知父组件
    if (onImageUpload) {
      onImageUpload(scene.id, imageUrl)
    }
      ...
      >
        ...
        {imageUrl ? (
          ...
          <div className="bg-white rounded-lg p-8 max-w-md text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">该场景暂无图片</h3>
