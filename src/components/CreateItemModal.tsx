...
function CreateItemModal({ onClose, onItemSelect, projectName }: CreateItemModalProps) {
  ...
  const saveItemToDatabase = async (task: ItemTask) => {
    if (!task.imageUrl) {
      console.warn('⚠️ 无法保存物品：缺少图片URL')
      alert('无法保存物品：缺少图片URL', 'error')
      return
    }
    
    if (!currentProjectName) {
      console.warn('⚠️ 无法保存物品：缺少项目名称')
      alert('请提供项目名称', 'error')
      return
    }
                    ...
                    if (file) {
                      ...
                      if (!itemName.trim()) {
                        ...
                        if (nameWithoutExt && 
                          ...
                          setItemName(nameWithoutExt.trim())
                        }
                      }
