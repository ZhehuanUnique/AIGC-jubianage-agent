              ...
              {[1, 2, 3, 4].map((item) => (
                <div
                  ...
                  className={`p-2 rounded cursor-pointer ${
                    item === rowId ? 'bg-purple-600' : 'bg-[#0a0a0a] hover:bg-[#2a2a2a]'
                  }`}
                >
          ...
          <div className="flex-1 flex flex-col p-6 overflow-y-auto">
            ...
            <div className="relative mb-6">
              <div className="flex gap-4 overflow-x-auto pb-4">
                {images.map((image, index) => (
                  <div
                    key={image.id}
                    className={`relative flex-shrink-0 w-32 h-20 bg-[#0a0a0a] border-2 rounded cursor-pointer ${
                  ...
                  >
                    ...
                    {selectedImages.includes(image.id) && (
                      <div className="absolute top-1 left-1 bg-purple-500 text-white text-xs px-1 rounded">
                        选定第{selectedImages.indexOf(image.id) + 1}张
                      </div>
                    )}
                  </div>
                ))}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select className="w-full px-4 py-2 bg-[#0a0a0a] border border-gray-700 rounded-lg focus:outline-none focus:border-purple-500">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative w-20 h-20 bg-[#0a0a0a] border border-gray-700 rounded">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  defaultValue="真人电影风格,画面描述:在华丽而高耸的古风塔顶,两个身着素雅古装的女子(&quot;我&quot;和&quot;闺密&quot;)并肩站立,凭栏远眺。下方是连绵的宫殿屋顶,显得宏伟而压抑。风吹动她们的发丝和衣袂。视觉重点:观众视线聚焦于两人眺望远方的落寞背影,通过大远景和引导线构图强调她们的渺小与孤独。整体呈现清冷的灰蓝色调,暗示压抑的心情。构图(Composition): 引导线构图。景别(Shot Scale): 大远景 (Extreme Long Shot)。机位 (Camera Position): 轴线后侧机位。角度(Angle): 俯视(High Angle)。镜头类型 (Lens Type): 广角镜头 (Wide-Angle Lens)。光线 (Lighting): 阴天的柔和天光(Overcast Soft Light)。"
                />
