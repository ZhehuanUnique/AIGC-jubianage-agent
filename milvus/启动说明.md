# Milvus Docker Compose 启动说明

## 🚀 快速启动

### 方法1：使用批处理脚本（Windows）

在项目根目录运行：
```bash
启动Milvus-DockerCompose.bat
```

### 方法2：手动启动

1. **进入 milvus 目录**：
   ```bash
   cd milvus
   ```

2. **启动服务**：
   ```bash
   docker-compose up -d
   ```

3. **查看服务状态**：
   ```bash
   docker-compose ps
   ```

4. **查看日志**：
   ```bash
   docker-compose logs -f
   ```

## 📋 服务说明

Docker Compose 会启动以下服务：

1. **etcd** - 元数据存储
   - 容器名：`milvus-etcd`
   - 端口：2379（内部）

2. **minio** - 对象存储
   - 容器名：`milvus-minio`
   - 端口：9000（API）、9001（控制台）
   - 访问：http://localhost:9001
   - 用户名：`minioadmin`
   - 密码：`minioadmin`

3. **milvus-standalone** - Milvus 主服务
   - 容器名：`milvus-standalone`
   - 端口：19530（gRPC）、9091（HTTP）
   - 健康检查：http://localhost:9091/healthz

## 🔧 常用命令

### 启动服务
```bash
cd milvus
docker-compose up -d
```

### 停止服务
```bash
cd milvus
docker-compose down
```

### 重启服务
```bash
cd milvus
docker-compose restart
```

### 查看服务状态
```bash
cd milvus
docker-compose ps
```

### 查看日志
```bash
cd milvus
# 查看所有服务日志
docker-compose logs -f

# 查看特定服务日志
docker-compose logs -f standalone
```

### 停止并删除数据
```bash
cd milvus
docker-compose down -v
```

## 🔍 验证服务

### 1. 检查容器状态
```bash
docker ps | findstr milvus
```

应该看到三个容器：
- `milvus-etcd`
- `milvus-minio`
- `milvus-standalone`

### 2. 检查 Milvus 健康状态
```bash
curl http://localhost:9091/healthz
```

应该返回 `OK`。

### 3. 检查端口
```bash
netstat -an | findstr "19530\|9091\|9000\|9001"
```

应该看到这些端口正在监听。

## ⚠️ 注意事项

1. **启动时间**：Milvus 完全启动需要 30-60 秒，请耐心等待
2. **数据持久化**：数据存储在 `milvus/volumes/` 目录下
3. **端口占用**：确保以下端口未被占用：
   - 19530 (Milvus gRPC)
   - 9091 (Milvus HTTP)
   - 9000 (MinIO API)
   - 9001 (MinIO 控制台)

## 🔄 配置后端服务

启动 Milvus 后，在 `server/.env` 文件中配置：

```env
VECTOR_DB_TYPE=milvus
MILVUS_HOST=localhost
MILVUS_PORT=19530
```

然后重启后端服务。

## 🐛 故障排查

### 问题1：端口被占用

**解决方案**：
1. 检查端口占用：
   ```bash
   netstat -ano | findstr 19530
   ```
2. 停止占用端口的进程，或修改 `docker-compose.yml` 中的端口映射

### 问题2：服务启动失败

**解决方案**：
1. 查看日志：
   ```bash
   docker-compose logs
   ```
2. 检查 Docker 资源：
   ```bash
   docker system df
   ```
3. 清理未使用的资源：
   ```bash
   docker system prune
   ```

### 问题3：数据丢失

**解决方案**：
- 数据存储在 `milvus/volumes/` 目录
- 不要删除此目录
- 使用 `docker-compose down`（不加 `-v`）停止服务，保留数据

---

**创建时间**：2026-01-03




