...
1. **登录系统** → 访问系统首页，点击"剧变时代Agent"按钮，使用您的账号登录
2. **创建项目** → 在"项目管理"页面，点击"新建场景"按钮，输入项目名称和描述
3. **输入剧本** → 在项目的"片段管理"页面，可以直接在文本框中输入剧本，或者上传`.docx`格式的剧本文件
4. **管理资产** → 系统自动分析并提取角色、场景、物品，可在"角色管理"、"场景管理"、"物品管理"页面进行管理
5. **生成分镜图片** → 在"分镜管理"页面，选择文生图模型和参数，点击"生成"按钮生成分镜图片（支持批量生成）
6. **配置视频参数** → 在"融图管理"页面，选择分镜图片，配置视频生成参数（模型、分辨率、时长、提示词）
7. **生成视频** → 点击"提交任务"生成视频，在"视频编辑"页面查看、编辑和导出生成的视频
...
- **首尾帧模式**：上传首帧 + 尾帧 + 提示词，生成视频（仅支持3.0pro模型）

## 👥 小组功能

### 创建小组

在"数据分析" → "用户管理" → "小组管理"中创建小组，将用户添加到小组后，小组成员可以共享"项目管理"中的文件夹。
